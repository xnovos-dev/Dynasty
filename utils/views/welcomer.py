...
class WelcomerPanel(ui.LayoutView):
    ...
    def __init__(self, bot: discord.Client, guild: discord.Guild, author: discord.Member):
        super().__init__(timeout=300)
        self.bot = bot
        self.guild = guild
        self.author = author

        settings = db.get_settings(guild.id) or {}
    ...
    def parse_variables(self, text: str, user: discord.Member):
        if not text:
            return text
    ...
    def build_view(self):
        ...
        container.add_item(ui.TextDisplay(
            f"### <:login:1473328547203518505> | Welcomer Panel\n"
            f"> **Status:** {status}\n"
            f"> **Channel:** {channel}\n"
            f"> **Auto Role:** {role}\n"
            f"> **Embed Title:** {self.data['embed'].get('title') or 'Not set'}\n"
            f"> **Embed Description:** {self.data['embed'].get('description') or 'Not set'}\n"
            f"### <:cog:1473326938926878760> | Available Variables\n"
            f"> {{user.mention}}\n"
            f"> {{user.name}}\n"
            f"> {{user.id}}\n"
            f"> {{member.number}}"
    ...
    async def enable_callback(self, interaction: discord.Interaction):
        db.set_welcomer_status(self.guild.id, "enabled")
        self.data["enabled"] = True
        self.build_view()
        await interaction.response.edit_message(view=self)

    async def disable_callback(self, interaction: discord.Interaction):
        db.set_welcomer_status(self.guild.id, "disabled")
    ...
    async def set_channel_callback(self, interaction: discord.Interaction):
        ...
        class ChannelModal(ui.Modal, title="Set Welcomer Channel"):
            ...
            async def on_submit(modal_self, modal_interaction: discord.Interaction):
                ...
                db.update_welcomer_channel(panel.guild.id, channel_id)
    ...
    async def set_role_callback(self, interaction: discord.Interaction):
        ...
        class RoleModal(ui.Modal, title="Set Auto Role"):
            ...
            async def on_submit(modal_self, modal_interaction: discord.Interaction):
                ...
                db.update_autorole(panel.guild.id, role_id)
    ...
    async def edit_embed_callback(self, interaction: discord.Interaction):
        ...
        class EmbedModal(ui.Modal, title="Edit Welcome Embed"):
            ...
            async def on_submit(modal_self, modal_interaction: discord.Interaction):
                ...
                db.update_embed(panel.guild.id, embed_data)
